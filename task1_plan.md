...
### Optional additions (not required by brief — add if time allows)

**Optional 1: Per-class accuracy extraction**

Not strictly needed since the confusion matrix diagonal already shows this visually. But if you want explicit numbers to reference in the report discussion:

```python
# Extract per-class accuracy from confusion matrix
per_class_acc = cm.diagonal() / cm.sum(axis=1)
for name, acc in zip(class_names, per_class_acc):
    print(f"{name}: {acc*100:.1f}%")
```

**Optional 2: Classification report (precision, recall, F1)**

The brief only asks for accuracy and confusion matrix. This gives richer metrics but is not required:
...
**Optional 3: Sample prediction visualisation**

Shows one test image per class with predicted vs true label underneath. Not required but makes the report look polished and makes the confusion matrix discussion more concrete:

```python
# Show one test image per class with predicted vs true label
fig, axes = plt.subplots(2, 5, figsize=(15, 6))
for i, ax in enumerate(axes.flatten()):
    # get one test image from class i, run inference, show predicted vs true
    ax.set_title(f"True: {class_names[true]}\nPred: {class_names[pred]}", fontsize=8)
plt.tight_layout()
plt.savefig('results/sample_predictions.png', dpi=150)
```

> These three additions take roughly 30 minutes to implement. Only do them if Task 1 code and results are already complete and you have time before the report deadline.
...
- [ ] Per-class accuracy extracted and discussed in report
- [ ] Classification report (precision, recall, F1) generated
- [ ] Sample prediction visualisation saved
